...
type ClassBooking struct {
	...
	ClassBookingDetail []*ClassBookingDetail `gorm:"constraint:OnUpdate:CASCADE,OnDelete:SET NULL;"`
}

type ClassBookingList []*ClassBooking

type ClassBookingDetail struct {
	ID               uint `gorm:"primaryKey;autoIncrement;<-:create"`
	ClassBookingID   uint
	ClassID          uint
	...
	ClassBooking ClassBooking  `gorm:"constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
}
